...
- **統一 scope resolver（`inScope`）+ 知識領域私有**：retrieve 用單一 `inScope(meta)` 組合 scope —— doc 若屬某 `kind='knowledge'` 領域，**只由 allow-list 決定**（繞過產品 taxonomy，所以「Cloud AP 產品線 **+** 行銷領域」能同時成立）；否則走產品 taxonomy filter。`knowledgeAreasAllowed`（陣列，含空陣列）= scoped 模式：領域 doc 非 allow-list 內就排除。`null`（內部 Ask）= 不 scoped、領域 doc 照產品路徑（維持原本全看得到）。**`/api/v1/search` 一律傳 `[]`（對外 API 絕不外洩部門知識）**；`/api/ask` workspace 傳 `scope.knowledge_areas`(+ 若 `scope.solution` 本身是領域則含它)。kind='knowledge' 的 slug 以 60s in-process 快取（不每次查 DB）。⇒ 「全產品 + 行銷」= 產品 Global + `knowledge_areas:['marketing']`；純 onboarding = `solution:'marketing'`。TaxonomyPicker `productOnly` 讓 workspace 產品 scope 只列 `kind='product'`。
- **向量索引 = HNSW**（00022，pgvector 0.8；取代 ivfflat lists=10 的低召回）。`match_threshold` 預設 **0.2**（CJK→EN 相似度偏低，靠 finalLimit+re-rank 控精度）。
- **PDF AI 抽取**：`extract-pdf-ai.ts` maxOutputTokens 32768 + 偵測 `finishReason==='MAX_TOKENS'` → 回 `{markdown,truncated}`，upload route 與 UI 對截斷出警告（大檔仍可能切，建議拆分）。
