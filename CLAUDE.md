...
- `product_lines` — solution_id (FK), ds_prefix, ds_images_folder_id, drive_folder_id, sort_order, **spec_footnote** (TEXT, NULL=不顯示) + **spec_footnote_translations** (JSONB `{ja, zh-TW, ...}`) — 渲染在最後一個 spec page 兩欄下方的備註（如 VPN Firewall 的 `*Note: Performance figures…` 註腳）。**qr_url_template** (TEXT, NULL=用 dict default) — 該產品線的 QSG URL 模板，`{model}` 會替換成 lowercase model_name。Cloud VPN FW / NVS / Switch / Extender 用 `doc.engenius.ai/.../<model>` 結構；Cloud AP & Camera 沒設、繼續用 `qr.engenius.ai/qsg/<model>` 短連結。Resolution priority：`product_translations.qr_url` (per-locale override) → `product_lines.qr_url_template` → `dict.defaultQrUrl`
