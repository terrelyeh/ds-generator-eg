...
export const API_KEY_MAP = {
  // All chat completions go through OpenRouter — one key reaches every
  // vendor. Two keys by decision: SpecHub (translation + battlecard) on
  // the first, EnGenie Ask on its own because it is the surface exposed
  // to public workspaces and the embed widget. Ask falls back to the
  // SpecHub key when unset. See Surface / KeyGroup in @eg/llm/openrouter.
