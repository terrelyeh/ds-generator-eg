...
76. **排程工作不能靠「跑到最後一行」來回報**（2026-09-16）——Vercel 在 `maxDuration` 直接殺掉函式，
    之後的心跳、log 一個都不會發生。`reindex-web` 兩次排程都這樣死，log 只剩一行 timeout、`job_heartbeats` 沒有列。
    要有**時間預算**：在預算內停止開始新工作、把沒做完的量寫進心跳的 detail（**`ok=false` 留給錯誤和「硬停時還在跑」**，2026-09-18 調整——預算用完而沒開始的來源算正常，否則警告每週都亮）。**光是「不再開始」不夠**——
    一個已經在跑的工作就能拖過上限，所以要有**硬停**：時間到就寫心跳、回應，不等它。
    能分批寫的（GitBook）要**邊做邊寫**，否則每次被殺都從頭來，工作量只要超過一次的額度就永遠追不上
    （而它本來就追不上——見 #78，GitBook 的爬取已經移出這支排程）。
    也要注意函式庫自己的重試：googleapis 預設重試、gaxios 會重試 timeout 的 GET，30s 的 timeout 實際是 90s。

77. **「消失的來源」的宇宙必須是這次 run 真的列舉出來的集合**（2026-09-16）——手動給的網址清單不是宇宙：
    「Add Article」傳一篇、逐列 Sync 傳一頁、cron 分批送。拿「這一類的全部」當宇宙，等於刪掉清單外的每一個來源，
    而且完全沒有症狀（頁面照常、toast 成功）。寫死的 fallback 清單也不是宇宙；**只列舉出一部分也不是**
    （SPA 解析不到會回空陣列，不會報錯）。就算列舉完整，宇宙也只是「屬於被列舉的那幾個分類」的來源，不是全部。

78. **一份永遠塞不完的工作量，不要硬留在排程裡「盡量做」**（2026-09-18）——`reindex-web` 第一次真的跑完的那輪
    就證明了四個 GitBook space 配 300s 只過得了兩個；輪替只是換誰被剩下，不是把它做完。
    這種狀況要把**「檢查」和「做」拆開**：檢查（讀 sitemap 比對索引日期）幾秒鐘、每週自動跑、
    結果存起來給人看；做（重爬）交給人在畫面上按，而且那條路要自己帶 deadline
    ——它現在是唯一會跑爬取的地方，沒有排程幫它踩剎車了。
    兩件容易做錯的判斷：**待辦的量不能算排程失敗**（否則健康檢查每週都亮，永遠亮的警告沒人看），
    但**檢查本身失敗要算錯誤**（沒人會被通知那個 space 有新頁）。還有：
    **給人看的數字必須是「做」那條路真的會處理的數字**——用同一支函式算，不要另寫一套判斷，
    不然兩邊會在其中一邊被修好的那天靜靜分岔。
