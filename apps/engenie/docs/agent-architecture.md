...
| **Multi-agent（多代理）** | 多個各有專長的 agent 協作（進階，見 Part 7） |
...
- **誤解 2：「這就是 Agent Teams / 多 agent。」** → **不是。** 這是「**一個** agent +
  一堆工具」。Agent Teams（多個 agent 協作）是更上層、更後面的事，你現在不需要（多 agent 怎麼設計見 Part 7）。
...
進得來、使用者買單」之後，再投資 B 的完整 provider 抽象。**B 才是「完整工具導向 agent」的終態**（真正的「多 agent」是更後面的事，見 Part 7），A 是過渡。
...
- **（未來）Stage 4 — 寫入類工具 / multi-agent**
  需人工確認的「改設定」工具；或多個專長 agent 協作（見 Part 7）。風險最高，獨立評估。
...
# Part 7 — 進階：多 Agent 架構（你還用不到，但先講清楚怎麼設計）

前面整份文件的結論是「**一個 agent ＋ 一堆工具**」就能走很遠。**Multi-agent**（多個各有專長的 agent 協作）是更後面、更進階的事 —— 它不是「更聰明」的免費升級，而是**用複雜度換取「專業化」與「隔離」**。這章講三件事：何時才真的需要、有哪幾種結構、套到 EnGenie 會長怎樣。

## 7.1 什麼時候才真的需要多 agent？

預設答案是「**還不需要**」。單一 agent 配上設計良好的工具集，能涵蓋的範圍比直覺大很多。只有當你**實際撞到**下面這些痛點之一，拆成多 agent 才划算：

| 觸發訊號 | 單一 agent 為什麼會痛 | 拆成多 agent 怎麼解 |
|---|---|---|
| **工具太多、選錯率上升** | 一個 agent 掛 15–20 個工具，system prompt 爆長、模型常選錯 | 按領域拆，每個 agent 只看自己 3–5 個工具，選得更準 |
...
| **子任務可平行** | 「比較三台規格 ＋ 同時查三台即時狀態」單一 agent 只能一步步排隊 | 主管 agent 把工作 fan-out 給多個 worker 同時跑 |
| **安全隔離** | 能打內網即時 API 的能力，跟對外公開問答塞在同一個 agent，風險面變大 | 把「碰得到內網」的 agent 獨立、收緊權限，跟公開 agent 隔開 |

> **反向訊號**：如果想做多 agent 只是因為「聽起來比較進階」，**別做**。每多一個 agent ＝ 多一輪（或多輪）LLM 呼叫，成本、延遲、失敗面全部往上疊。

## 7.2 兩種主要結構（topology）

多 agent 怎麼接，主流就兩種骨架；其餘變形多半從這兩種衍生。

**(A) 主管–工人（Supervisor / Orchestrator-Worker）** —— 一個「主管 agent」收下需求，**自己不做領域工作**，而是判斷該交給哪個（哪幾個）專長子 agent、派工、收回結果、綜合成最終答案。
...
```
   ...
   使用者問題 ──▶ │  看懂需求 → 決定派給誰 → 收齊結果 → 綜合成答案    │
                  └──────┬──────────────┬──────────────┬──────────┘
                   ...
                   └ search_kb     └ device_status / event_logs …
                      ↑ 各自有自己的 prompt、工具子集、甚至模型
```

**(B) 子 agent 當工具（Agent-as-Tool）** —— 這是**從本文單一 agent 設計最自然的長法**：回頭看 §3.2 的 `Tool` 介面，那裡的表已經列了一種工具叫「**LLM 型工具 / 子 agent**」。一個子 agent 其實就是一個 `run()` 內部跑著自己 agent loop 的工具 —— 上層 agent 像呼叫任何工具一樣呼叫它。

```
上層 Agent 的工具清單：
   ├─ search_knowledge_base        ← 純程式工具
   ├─ get_device_status            ← 純程式工具
   └─ ask_network_specialist       ← 子 agent 包成的工具：run() 裡是另一個 agent loop
                                    （帶自己的 prompt ＋ 自己的即時 API 工具子集）
```

> **關鍵**：(B) 不是新架構，是 §3.2 Tool 抽象的一個 case。把單一 agent 的 Tool 介面做好，未來要多 agent 就是「**把某個專長包成一個 run() 是 agent loop 的工具**」，**不用重寫**。

| | (A) 主管–工人 | (B) 子 agent 當工具 |
|---|---|---|
| 誰決定派工 | 主管 agent 動態判斷 | 上層 agent 把子 agent 當工具，照選工具邏輯叫 |
| 改動量 | 要新增 supervisor 層 ＋ 派工協定 | 幾乎零新概念，沿用 Tool 介面 |
| 適合 | 專長多（3＋）、需要動態組合與平行派工 | 只想隔出 1–2 個專長子 agent（EnGenie 的起點） |

**另外兩種變形（知道即可）**：固定**流水線**（planner → executor → reviewer，順序寫死、好控）、**對等交接**（一個 agent 把控制權「轉接」給另一個，常見於客服路由）。EnGenie 用不太到，先略。

## 7.3 套到 EnGenie：會長怎樣

如果哪天 EnGenie 的單一 agent 真的撐不住要拆，最自然的切法是**按「安全邊界」拆，而不是按「聰明」拆**：
...
主管 agent 收到「**ECW536 規格 ＋ 它現在線上嗎**」→ 規格題派給知識 Agent、即時題派給即時網路 Agent，再把兩邊合成一個答案（標清楚哪段是知識、哪段是即時數據與查詢時間）。

> **這跟你們現有設計接得起來**：「哪個 workspace 能用哪個專長 agent」**就是 §3.5 per-workspace 權限**再上一層 —— 公開 widget 的 workspace 只給「知識 Agent」，內部 workspace 才給「即時網路 Agent」。多 agent **不需要另一套權限模型**，沿用 `ask_workspaces.scope` 的思路即可。

## 7.4 多 agent 多出來的工程問題（誠實面）

拆成多 agent 不是免費的，它把幾個原本沒有的難題端上桌：

1. **上下文怎麼傳（最難的一題）** —— 子 agent 預設**看不到**完整對話。主管要決定「往下傳什麼、子 agent 往上回什麼」。傳太少 → 子 agent 缺資訊答錯；整包往下傳 → token 爆炸。這是多 agent 的頭號工程成本。
2. **成本與延遲相乘** —— 每個子 agent 各跑一輪自己的 loop（多次 LLM 呼叫），兩層的樹很容易變成單一 agent 的數倍 token 與時間。
3. **錯誤會累積** —— 主管派錯 ＋ 子 agent 讀錯，誤差一層層疊上去，也更難定位是哪一層出包。
4. **最終答案誰負責、一致性** —— 兩個子 agent 給出矛盾資訊時，主管要負責調和；跨 agent 的**來源引用**也更難串。
5. **可觀測性** —— 原本一條線性 trace 變成一棵樹，log／tracing 要記「哪個 agent 叫了哪個、帶什麼、回什麼」整棵樹，否則出事查不到。
6. **遞迴護欄** —— 在原本「步數上限」之外，還要加「**agent 深度上限**」，避免 agent 叫 agent 無限往下。

> 一句話：**多 agent ＝ 用「成本 × 延遲 × 失敗面 × 除錯難度」換「專業化 ＋ 隔離」。** 沒撞到 7.1 的痛點前，這筆交易划不來。

## 7.5 務實建議

1. **先把單一 agent 做扎實**（本文 Part 3–5）—— 它能走的路比你想的遠，多數需求加個工具就解決。
2. **真要拆，第一刀切在「安全邊界」** —— 把「碰得到內網即時 API」的能力獨立成一個收緊權限的 agent，跟對外知識 agent 隔開。光是**隔離價值**就回本，不必等到「更聰明」。
3. **用 (B) 子 agent 當工具落地**，沿用 §3.2 Tool 抽象，**別**一上來就蓋動態 supervisor。等真的有 3＋ 個專長、需要動態組合與平行，再升級成 (A)。
4. **保持淺** —— 1 個主管 ＋ 一排平行專長（深度 ≤ 2），避免深層 agent 樹。
5. **排在 §5 的 Stage 4 之後** —— 單一 agent 穩了、且實際感受到 7.1 的某個痛點，再啟動。

> **好消息**：因為本文的單一 agent 設計已經用了乾淨的 **Tool 抽象**（§3.2）＋ **per-workspace scope**（§3.5），未來要走多 agent，是「**把一個專長包成工具**」，不是打掉重練 —— 這套架構**天生就是 multi-agent-ready**。
