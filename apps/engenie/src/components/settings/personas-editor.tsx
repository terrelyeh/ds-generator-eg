  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-[28px] font-bold tracking-tight">Ask Personas</h1>
        ...
        <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-base flex items-center gap-2">
                        {p.name}
                        {p.is_default && (
                          <span className="rounded bg-muted px-1.5 py-0.5 text-[13px] font-medium text-muted-foreground">Built-in</span>
                        )}
                        {p.updated_at && (
                          <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[13px] font-medium text-amber-700">Customized</span>
                        )}
                      </CardTitle>
              ...
              {editingId === p.id ? (
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-[80px_1fr] gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-md border border-input bg-background px-2 py-1.5 text-center text-[23px]"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-md border border-input bg-background px-3 py-1.5 text-base"
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={10}
                      className="w-full rounded-md border border-input bg-background px-3 py-2 text-base font-mono leading-relaxed"
                    />
                  </div>
                </CardContent>
              ) : (
                <CardContent>
                  <details className="group">
                    ...
                    <pre className="mt-2 rounded-md bg-muted p-3 text-sm font-mono leading-relaxed text-muted-foreground whitespace-pre-wrap">
                      {p.system_prompt}
                    </pre>
                  </details>
                </CardContent>
          ...
          {showAdd ? (
            <Card className="shadow-sm border-dashed border-engenius-blue/30">
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-[80px_1fr_1fr] gap-3">
                  ...
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">ID (slug)</label>
                    <input
                      value={newId}
                      onChange={(e) => setNewId(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))}
                      placeholder="e.g. partner"
                      className="w-full rounded-md border border-input bg-background px-3 py-1.5 text-base font-mono"
                    />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">Name</label>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-muted-foreground/20 py-4 text-base font-medium text-muted-foreground hover:border-engenius-blue/30 hover:text-engenius-blue transition-colors"
            >
      ...
      <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-base text-blue-800">
        <strong>Tip:</strong> Each persona can be used via the Ask page or directly via API:
        <code className="ml-1 rounded bg-blue-100 px-1.5 py-0.5 text-sm font-mono">POST /api/ask {"{"}&quot;persona&quot;: &quot;sales&quot;, ...{"}"}</code>
      </div>
    </div>
  );
}
