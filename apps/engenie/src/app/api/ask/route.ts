...
1. Use inline citations like [1] to reference source documents — the number only: write [7], never [Source 7] (the ids on the <source> elements are labels, not citation text). Rules: place ONE citation at the END of a paragraph or key claim (not after every sentence). Maximum 2 citations per paragraph. Never stack multiple citations together like [1, 3, 4, 5] — pick the single most relevant source.
