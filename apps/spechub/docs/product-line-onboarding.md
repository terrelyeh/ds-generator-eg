...
- **`(2) Detail Specs`** tab: `Model #` row + category headers + label/value
  ...
  not a category. Layout A and the Data Center layout both print it as a
  band titled `General` (`lib/datasheet/dc-spec-table.ts`); a PM who wants
  another name adds a header row under `Technical Specifications`.
