...
- **`(2) Detail Specs`** tab: `Model #` row + category headers + label/value
  ...
  Rows directly under the `Technical Specifications` row, before any header,
  are filed as category **`General`**: that row is the parser's start marker,
  not a category. The Data Center layout prints a band for every named
  category but none for `General`, because its page title already says
  Technical Specifications (`lib/datasheet/dc-spec-table.ts`).
...
| **Data Center** | Edge Network Appliances, AI Servers | dedicated component `preview/[model]/datacenter-preview.tsx`; photo hero (`HERO_PHOTO`, shared by both lines) + 8 chip features, shared EDCC page, full-width spec table with a band per named sheet section (none for `General`), 2 hardware renders, Contact-Us QR |
