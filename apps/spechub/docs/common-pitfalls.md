...
## 從 datasheet 封面照移入（2026-09-08）

## 67. 量一個區域的**平均值**，會把「文字讀不讀得到」這件事藏起來（已修）

**症狀**：使用者回報 Data Center 封面的內文看不清楚。量測卻說沒問題——文字區的
平均亮度換算成白字對比是 **18:1**，低於 3:1 的像素只有 0.5%。數字漂亮，畫面糊。

**原因**：那 0.5% 不是隨機散布的，是**幾條很窄的垂直亮帶**——機櫃的 LED 燈條。
文字剛好橫跨其中一條。平均值把它們稀釋掉了，而人眼看到的是筆畫在亮點上散開。

**通則**：判斷「文字壓在圖上讀不讀得到」要看**尾端**（p99、最差值、低於門檻的比例），
不能看平均。平均只回答「這張圖暗不暗」，那不是同一個問題。
`lib/datasheet/cover-photo-check.ts` 就是照這個寫的——它只回報尾端。

**而且不要只相信量測**：這次是把版型的 hero 區用同一份 CSS、同一張圖、真實 pt 級數
**實際渲染出來**才看見的。量測能抓到眼睛會忽略的，渲染能抓到量測會平均掉的，兩個都要。

**連帶的第二個原因**：`.hero-overview` 當時是 `WT.light`。那是**唯一**有內文壓在
照片上的封面，而 300 是刻度裡最細的一階——在原本的純色漸層底上沒事，換成照片才不行。
**底從色塊換成影像時，壓在上面的字重要重新檢討。**

## 68. 一鍵刪掉的東西，通常是唯一一份（已修）

封面照候選的 `×` 原本是單擊即刪，而且會連 Storage 檔案一起移除。面板第一次被使用者
打開就誤刪了一張。救得回來純粹是因為原始檔還在暫存目錄。

**當時的判斷錯在**：把「移除一個候選」想成輕量操作。但清單上限只有 3 張，被刪的那張
**通常就是任何人手上唯一的一份**。

**通則**：一個動作要不要確認，看的不是它在 UI 上有多小，而是**失敗的代價可不可逆**。
尤其當它和「切換」（安全、可逆）這種操作**並排在同一個面板裡**、目標又只有 16px 的時候。

## 79. 同一列的第二個寫入者漏帶一個欄位，等於把它清成 null（2026-09-18，已修）

規格備註上線隔天 MKT 就回報：**日文版的備註翻譯完、PDF Preview 還是英文**。

翻譯編輯器有兩個按鈕會寫同一列 `product_translations`——**Save 和 Preview**
（預覽畫的是「已存的內容」，所以它也得先存）。Preview 的 payload 是規格備註存在
之前寫的，從來沒有加上 `spec_notes`；而 API 當時把每個欄位都寫一次，
`spec_notes?.trim() || null` 於是把「沒給」讀成「設成 null」。

所以流程變成：翻好日文備註 → 按 Preview → **剛翻的字被刪掉** → 預覽頁讀不到翻譯 →
退回產品的英文備註 → PDF 印英文。整條路上沒有任何錯誤、沒有 log，
編輯器畫面上那段日文還在（state 沒變），重新整理才會發現不見了。
正式資料裡 `product_translations.spec_notes` **每一列都是 null**。

**兩層修法**：
① 編輯器只有**一個 payload builder**（`editorPayload()`），Save 和 Preview 共用——
下一個語系欄位不可能只加到其中一邊。
② API 改成**「body 沒提到的欄位就不寫」**（`lib/translate/product-upsert.ts`）——
這一層才是不會再被忘記的那層：比編輯器早寫好的呼叫端也安全了。
「清空」仍然有效，因為清空是一個**值**不是「沒有」：編輯器一律送出該欄位，空字串存成 null。

**通則**：一張表有兩個以上的寫入者時，「整列覆寫」的語意就是個陷阱——
