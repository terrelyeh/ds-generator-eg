...
- **應用情境圖可以在圖上標字**：點圖片放標記點、拖曳文字避開設備，字由版型排（不是畫進圖裡），
  所以改得動、之後也能翻譯
- **文案可以用 AI 從規格表起草**：封面（主標、分類、Overview、Features、架構圖說明）
  和情境圖旁邊那幾段。**它只准引用這份文件規格表上有的東西**——每一點會標出依據的規格列，
  沒有規格依據的句子標成橘色要你自己確認，撐不起來的說法會列出來說「缺哪一條」。
  一格一格填進表單，**不會自動存檔**
- **從既有型號帶入時，封面文案一起帶**（主標、副標、分類、Overview、Features），
  **只填空的**——加第二台不會蓋掉你為第一台寫的字
...
- **依產品線 5 種版型變體**：
  ...
  - **Data Center 青色 `#09909d`**（Edge Network Appliance / AI Server）— 照片 hero 封面 + 8 個賣點區塊、
    全系列共用的 EDCC 管理平台頁、全寬單機規格表（依 sheet 分段，每段一條灰色標題，第一段 General
    以 Model Name / Model Number 開頭；自動分頁，短的段落不會被拆到兩頁）、Hardware Overview 最多 2 張圖、Contact Us QR。
