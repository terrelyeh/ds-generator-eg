...
export function radioPatternSlots(src: RadioPatternSource): RadioPatternSlot[] {
  ...
  // Station Outdoor plots per antenna port for EVERY model — it doesn't split
  // by radio the way Broadband does. ENS621EXT is dual-radio and still prints
  // Port1/Port2, so don't "improve" this into a band test: these are
  // directional units whose pattern is a property of the port, not the band.
  if (src.category === "Station APs") {
    return slotsFor(["Port1", "Port2"]);
  }
